...
INSERT INTO platillos (nombre, precio, disponible, categoriaId) VALUES ('Cupcake de fresa', 75, 5, 5);
...
-- 6. Encontrar la categoría con el máximo ID:
   SELECT * FROM categoria ORDER BY id  DESC LIMIT 1;
   


-- 7. Listar las primeras 3 categorías ordenadas alfabéticamente:
   SELECT * FROM categoria ORDER BY NOMBRE LIMIT 3;
 


-- 8. Listar todas las categorías que contengan la letra 'a':
SELECT * FROM categoria WHERE nombre LIKE '%b%';
...
SELECT AVG(precio) As media_precios FROM platillos;
...
-- 13. Sumar el precio de todos los platillos de una categoría específica (e.g., Postres):
	SELECT categoriaID,SUM(precio) AS Precio_postres
    FROM platillos 
   WHERE categoriaID = 5;
...
-- 16. Contar el número de reservaciones para cada fecha:
   SELECT fecha,COUNT(*) as num_reservaciones_fecha
   FROM reservaciones
   GROUP BY fecha;
...
SELECT *
FROM clientes
WHERE apellido not LIKE '%Pérez%' AND apellido NOT LIKE '%López%' AND apellido NOT LIKE '%Gómez%';

SELECT *
FROM clientes
WHERE apellido NOT IN ('Pérez','López','Gómez');
...
SELECT categoriaId, COUNT(*) AS Cantidad_platillos,AVG(precio) AS Precio_promedio_platillos
FROM platillos
WHERE disponible = 1
GROUP BY categoriaId
HAVING COUNT(*) > 1
